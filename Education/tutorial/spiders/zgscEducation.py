...
class EduSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for page in range(1,6):
            URL = 'http://www.chinasilkmuseum.com/list_151.aspx?page={}'.format(page)
        # print(URL)
    ...
    def parse(self, response):
       ...
       #  print(response.xpath('/html/body/form/div[3]/table/tr/td/table/tr[3]/td/table/tr[3]/td/table/tr/td[3]/div/span[2]/div/table[1]/tr[1]'))
        for line in response.xpath('/html/body/div[1]/div/div[2]/div/div/ul/li'):  # 教育信息爬取
            # print(1)
            item = eduItem()
            item['mid'] = index
            # print(item['mid'])
            #                             '/html/body/div[3]/div/div/ul/li[17]/a'
            item['name'] = line.xpath('./p/a/text()').extract()[0]
            print(item['name'])
            # print(line.xpath('./h2/a/@href').extract())
            item['url'] = "http://www.chinasilkmuseum.com"+line.xpath('./p/a/@href').extract()[0]
            print(item['url'])
