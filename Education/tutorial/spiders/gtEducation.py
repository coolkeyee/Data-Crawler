...
class EduSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for page in range(1,3):
            URL = 'http://www.gthyjng.com/shjy/jyhd/index_{}.htm'.format(page)
        # print(URL)
    ...
    def parse(self, response):
       ...
       #  print(response.xpath('/html/body/form/div[3]/table/tr/td/table/tr[3]/td/table/tr[3]/td/table/tr/td[3]/div/span[2]/div/table[1]/tr[1]'))
        for line in response.xpath('/html/body/div[4]/div/div[2]/div[2]/ul/li'):  # 教育信息爬取
            # print(1)
            item = eduItem()
            item['mid'] = index
            # print(item['mid'])
            ...
            item['name'] = line.xpath('./h1/text()').extract()[0].replace('\t','').replace('\n','')
            print(item['name'])
            # print(line.xpath('./h2/a/@href').extract())
            item['url'] = line.xpath('./p/a/@href').extract()[0]
            print(item['url'])
