...
class EduSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for page in range(1, 5):
            URL = 'https://www.wmhg.com.cn/news_activity/p/{}.html'.format(page)
            # print(URL)
    ...
    def parse(self, response):
       ...
       # ' / html / body / div / div / div[2] / div[2] / ul[1] / li[2] / div / a / h3 / strong'
        for line in response.xpath('/html/body/div[6]/div/div[2]/div/div/div[1]/div'):  # 教育信息爬取
            # print(1)
            item = eduItem()
            item['mid'] = index
            # print(item['mid'])
            #                             '/html/body/div[3]/div/div/ul/li[17]/a'
            item['name'] = line.xpath('./div[2]/div[1]/a/text()').extract()[0]
            print(item['name'])
            # print(line.xpath('./h2/a/@href').extract())
            item['url'] = "https://www.wmhg.com.cn" + line.xpath('./div[2]/div[1]/a/@href').extract()[0]
            print(item['url'])
