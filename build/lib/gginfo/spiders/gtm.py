...
class GtmSpider(scrapy.Spider):
    name = 'gtm'
    # allowed_domains = ['http://www.gthyjng.com/gcww/wwjs/tdgmsq/']
    # start_urls = ['http://http://www.gthyjng.com/gcww/wwjs/tdgmsq//']

    def start_requests(self):
        url = "http://www.gthyjng.com/gcww/wwjs/tdgmsq/"
        count = 0
    ...
    def parse(self, response):
        if (response.status == '404'):
            return
        detailurl = []
        detailurl=response.css('body > div.con.pad_t25 > div > div.news_r.f_r > div.news_r_img > ul > li > a::attr(href)').extract()
